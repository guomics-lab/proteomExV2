...
toPlotHmap<-proSel_NAFill[ImpPros[1:60],]
...
names(Median_LocY)<-names(Median_LocX)<-c('N','L','H',"PC","CC")
...
barplot(rep(1,length(heatmap_colors)), col = heatmap_colors)
...
for (i in 1:length(punchLocs)){
  ...
  plot3d(contourLines[[i]][,1],contourLines[[i]][,2]*2, i*500, type="l",add=T,col="#D1ACC580")
  # polygon3d(contourLines[[i]][,1],contourLines[[i]][,2],rep(i*500,nrow(contourLines[[i]])),col="#D1ACC580")
  # plot3d(punchLocs[[i]][,1], punchLocs[[i]][,2], i*500, type="s", col=kmeans_model$cluster[row.names(punchLocs[[i]])], size=1.5,add=T)
  arrow3d(p0=c(clusterX[3],clusterY[3],clusterZ[3]),p1=c(clusterX[2],clusterY[2],clusterZ[2]), type = "rotation",col="#D1ACC580")
  arrow3d(p0=c(clusterX[2],clusterY[2],clusterZ[2]),p1=c(clusterX[4],clusterY[4],clusterZ[4]), type = "rotation",col="#D1ACC580")
  arrow3d(p0=c(clusterX[4],clusterY[4],clusterZ[4]),p1=c(clusterX[1],clusterY[1],clusterZ[1]), type = "rotation",col="#D1ACC580")
  # arrow3d(p0=c(clusterX[4],clusterY[4],clusterZ[4]),p1=c(clusterX[5],clusterY[5],clusterZ[5]), type = "rotation",col=5)
  pch3d(punchLocs[[i]][,1], punchLocs[[i]][,2]*2, rep(i*500,nrow(punchLocs[[i]])),pch=kmeans_model$cluster[row.names(punchLocs[[i]])]+20, cex=0.5,add=T,bg = colIDs[sapply(row.names(punchLocs[[i]]),toPlotPar)[3,]])
}
view3d(theta=-5, phi=-60, fov=0)
}
...
names()<-names(angleAll)<-unique(relations)


{pdf("PathPlot_dark.pdf",width=36,height=15)
  plot(0,0,type='n',ylim=c(1200,200),xlim=c(1000,0),axes=F,xlab="",ylab="")
  par(mar=c(0,0,0,0))
for(i in 1:length(geneName)){
  text(geneLocy[i],geneLocx[i],labels = unlist(strsplit(geneName[i],", "))[1],srt=45,pos=1)
}
for(i in 1:length(relations)){
  arrows(x0=yLoc[src[i]],x1=yLoc[dst[i]],
         y0=xLoc[src[i]],y1=xLoc[dst[i]]
         ,length=0.1,code = 2
...
for (i in 1:length(uniNameSel)){
  ...
  # plot(0,0,type='n',xlim=c(0,imgWidth),ylim=c(imgHeight,0),axes=F,xlab="",ylab=""
  #      ,main="")#row.names(proMat)[m]
  polygon(geneLocySel[i]+contourLines[[1]][,1]/70-50,
          geneLocxSel[i]+contourLines[[1]][,2]/20-50,
          col="#684784",border=NA)
  pchAll<-rep(21,length(proSelZ))
  pchAll[is.na(proSelZ)]<-21
  colAll<-colGenMax(proSelZ,heatmap_colors)
  colAll[is.na(colAll)]<-NA
  points(geneLocySel[i]+punchLocs[[1]][,1]/70-50,
         geneLocxSel[i]+punchLocs[[1]][,2]/20-50,
         cex=1,pch=pchAll,bg=colAll,col=NA)
}
dev.off()
}
